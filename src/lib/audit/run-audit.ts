...
import { probeSearchConsole } from "./probes/search-console";
...
export async function executeFullAudit(input: {
  ...
  const searchConsolePromise = probeSearchConsole(input.websiteUrl);
  ...
  const searchConsole = await searchConsolePromise;
  ...
  if (searchConsole.state !== "observed") missing.push("Authorized Search Console keyword performance");
  if (baseReport.googleLocal?.rawError) missing.push("Some Google search captures");
  return enrichReportWithLlm({
    businessName: input.businessName,
    websiteUrl: input.websiteUrl,
    zipCode: input.zipCode,
    baseReport: { ...baseReport, searchConsole, googleAIMode, coverage: { status: missing.length ? "partial" : "complete", missing }, sections: baseReport.sections.map(s => s.key === "ai" ? { ...s, measured: googleAIMode.state === "observed", summary: googleAIMode.state === "observed" ? `Google AI Mode answer captured. Brand mentioned: ${googleAIMode.mentioned ? "yes" : "no"}; website cited: ${googleAIMode.cited ? "yes" : "no"} in this sample. ChatGPT remains unmeasured.` : "No usable Google AI Mode answer collected. This is not proof of brand absence.", dataSource: "Google AI Mode via SerpAPI; see retained answer and sources" } : s) },
  });
}
