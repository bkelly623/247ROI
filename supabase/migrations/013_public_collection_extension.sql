...
create function public.audit_reserve_public_collection(p_session uuid,p_kind text,p_sample_key text,p_query text,p_request jsonb,p_upper_cost bigint)
...
begin
 ...
 if not exists(select 1 from public.audit_collection_budgets_v2 where id='public-chatgpt-remaining-088000' and purpose='website' and ceiling_micros=88000 and authorized_micros+p_upper_cost<=ceiling_micros) then return false; end if;
...
create function public.audit_record_public_collection(p_session uuid,p_kind text,p_sample_key text,p_outcome jsonb)
returns boolean language plpgsql security invoker set search_path=public as $$
declare j public.audit_jobs_v2;
begin
 if jsonb_typeof(p_outcome) is distinct from 'object' or coalesce(p_outcome->>'status','') not in ('observed','unmeasured','error','blocked') or octet_length(p_outcome::text)>262144 then return false; end if;
 select * into j from public.audit_jobs_v2 where idempotency_key='public-collection:'||p_session::text||':'||p_kind||':'||p_sample_key for update;
 if not found or j.requests_reserved<>1 then return false; end if;
 if j.public_chatgpt_outcome->>'status'<>'uncertain' then return j.public_chatgpt_outcome=p_outcome; end if;
 update public.audit_jobs_v2 set public_chatgpt_outcome=p_outcome,updated_at=now() where id=j.id;
 return true;
end; $$;
revoke all on function public.audit_reserve_public_collection(uuid,text,text,text,jsonb,bigint),public.audit_record_public_collection(uuid,text,text,jsonb) from public,anon,authenticated;
grant execute on function public.audit_reserve_public_collection(uuid,text,text,text,jsonb,bigint),public.audit_record_public_collection(uuid,text,text,jsonb) to service_role;
